...
export default function QADashboardSV() {
  ...
  const loadData = async (refresh = false) => {
    try {
      ...
      console.error("Kunde inte ladda Q&A-data:", error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const exportData = () => {
    if (!data) return;
    const csv = [
      ["Fråga", "Antal", "Besvarad", "Botar", "Först frågad", "Senast frågad"],
      ...data.topQuestions.map(q => [
        q.question,
        q.count,
        q.answered ? "Ja" : "Nej",
        q.bots.join("; "),
        new Date(q.firstAsked).toLocaleDateString("sv-SE"),
        new Date(q.lastAsked).toLocaleDateString("sv-SE"),
      ]),
    ]
      .map(row => row.join(","))
      .join("\n");
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Kunde inte ladda Q&A-data</p>
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
            <div className="mb-4 lg:mb-0">
              <h1 className="text-4xl font-bold uppercase tracking-wider text-black mb-2">
                Q&A ANALYS DASHBOARD
              </h1>
              <p className="text-gray-600">
                Omfattande analys av alla frågor från dina botar
              </p>
            </div>
          ...
          <div className="flex flex-wrap gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="minimal-input py-2 px-3 text-sm"
              >
                <option value="all">Alla botar ({data.bots.length})</option>
                {data.bots.map(bot => (
        ...
        >
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-black">
                  {data.summary.totalQuestions.toLocaleString("sv-SE")}
                </p>
              </div>
              <MessageSquare className="w-8 h-8 text-gray-400" />
            </div>
            <p className="text-xs text-gray-500 mt-2">
              {data.summary.uniqueQuestions} unika
            </p>
          </div>
          ...
          <div className="minimal-card animate-pulse-shadow">
            ...
            <p className="text-xs text-gray-500 mt-2">
              {timeRange === "24h" ? "Senaste 24h" : timeRange === "7d" ? "Senaste 7 dagar" : timeRange === "30d" ? "Senaste 30 dagar" : "All tid"}
            </p>
          </div>
        </motion.div>
        ...
        >
          <div className="inline-flex bg-white rounded-xl shadow-sm p-1">
            {[
              ...
              { id: "top", label: "Vanligaste frågor", icon: TrendingUp },
              { id: "unanswered", label: "Obesvarade", icon: AlertCircle },
              { id: "trending", label: "Trending", icon: Zap },
              { id: "bots", label: "Per bot", icon: Bot },
            ].map(tab => (
              <button
                key={tab.id}
        ...
        <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      {q.avgResponseTime && (
                        <div className="text-right">
                          <p className="text-xs text-gray-500">Snitt-svar</p>
                  ...
                  {data.recentQuestions.slice(0, 10).map((q, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500">
                        <span>{new Date(q.lastAsked).toLocaleString("sv-SE")}</span>
                        {q.answered ? (
          ...
          {activeView === "top" && (
            ...
            >
              <h3 className="text-lg font-bold uppercase tracking-wider text-black mb-4">
                Alla vanligaste frågor ({filteredQuestions(data.topQuestions).length})
              </h3>
              <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                            ...
                            <>
                              ...
                              {q.avgResponseTime && (
                                <span className="flex items-center gap-1 text-gray-600">
                                  ...
                                  {formatTime(q.avgResponseTime)} snitt
                                </span>
                              )}
                            </>
                          ) : (
                        ...
                        <div className="mt-2 flex items-center gap-2 text-xs text-gray-500">
                          <span>Först: {new Date(q.firstAsked).toLocaleDateString("sv-SE")}</span>
                          <span>•</span>
                          <span>Senast: {new Date(q.lastAsked).toLocaleDateString("sv-SE")}</span>
                        </div>
                      </div>
                    </div>
                  </motion.div>
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-bold uppercase tracking-wider text-black">
                  Obesvarade frågor ({data.unansweredQuestions.length})
                </h3>
              </div>
              {data.unansweredQuestions.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">
                    Bra jobbat! Alla frågor har blivit besvarade.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {filteredQuestions(data.unansweredQuestions).map((q, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-600">
                        ...
                        <span>
                          Senast frågad: {new Date(q.lastAsked).toLocaleDateString("sv-SE")}
                        </span>
                        <span className="text-gray-500">
                          {q.bots.join(", ")}
                        </span>
          ...
          {activeView === "trending" && (
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-bold uppercase tracking-wider text-black">
                  Trending frågor ({data.trendingQuestions.length})
                </h3>
              </div>
              {data.trendingQuestions.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">
                    Inga trending frågor i denna tidsperiod.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-3 text-xs">
                            ...
                            <span className="flex items-center gap-1 font-bold text-orange-600">
                              {q.trend} Trending {q.trend === "↑↑" ? "starkt" : "uppåt"}
                            </span>
                          </div>
                        </div>
                      </div>
              ...
              {data.botStats.map((bot, idx) => (
                ...
                >
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button
                      onClick={() => router.push(`/sv/dashboard/${bot.botId}`)}
                  ...
                  {bot.unansweredQuestions.length > 0 && (
                    <div>
                      <h4 className="text-sm font-bold uppercase tracking-wider text-red-600 mb-3">
                        Obesvarade ({bot.unansweredQuestions.length})
                      </h4>
                      <div className="space-y-2">
                        ...
                        {bot.unansweredQuestions.length > 3 && (
                          <p className="text-xs text-gray-500 text-center">
                            +{bot.unansweredQuestions.length - 3} till
                          </p>
                        )}
                      </div>
                    </div>
