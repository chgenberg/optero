...
export default function DueDiligence() {
  ...
  useEffect(()=>{
    try {
      const raw = localStorage.getItem('dd_wizard');
      if (raw) {
  ...
  useEffect(()=>{
    try {
      localStorage.setItem('dd_wizard', JSON.stringify({ url, quickKpi, integrations, blueprint, problem, selectedTemplates }));
    } catch {}
  ...
  const buildBots = async () => {
    ...
    if (!validate()) return;
    await saveQuickKpis();
    // Optional: refresh scoring (non-blocking)
  ...
  useEffect(() => {
    // Fetch embed snippets for created bots
  ...
  const onNextFromStep2 = async () => {
    // Re-run analyze including uploaded documents
    await analyze();
    setCurrentStep(3);
  };
