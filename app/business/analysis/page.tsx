            ...
            {question.type === "select-or-text" && (
              <div className="max-w-2xl mx-auto space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {question.options?.map((option, i) => (
                    <button
                      key={option}
                      onClick={() => setCurrentAnswer(option)}
                      className={`relative p-4 rounded-xl border-2 transition-all transform hover:scale-105 group overflow-hidden ${
                        currentAnswer === option
                          ? "border-gray-900 bg-gray-50"
                          : "border-gray-200 hover:border-gray-400"
                      }`}
                      style={{ animationDelay: `${i * 0.05}s` }}
                    >
                      <span className="relative z-10 font-medium text-gray-900 text-sm">{option}</span>
                      {currentAnswer === option && (
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    value={question.options?.includes(currentAnswer) ? "" : currentAnswer}
                    onChange={(e) => setCurrentAnswer(e.target.value)}
                    className="w-full pl-16 pr-4 py-4 border-2 border-gray-200 rounded-xl focus:border-gray-900 focus:outline-none transition-all"
                    placeholder={question.placeholder || "Skriv ditt eget svar..."}
                  />
                </div>
              </div>
            )}
