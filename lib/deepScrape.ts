...
export type DeepScrapeResult = {
  ...
  url: string;
  pagesScraped: number;
  pages: DeepScrapePage[];
  analysis: any;
  rawText: string;
};

// Resilient fetch with timeout and minimal retries
async function fetchWithRetry(input: string, init: RequestInit & { timeoutMs?: number } = {}) {
  const maxRetries = 2;
...
export async function deepScrapeSite(url: string, documentContent?: string): Promise<DeepScrapeResult> {
  ...
  // Fallback: crawl internal links from main page (limit to 50)
  if (sitemapUrls.length === 0) {
    const baseUrl = new URL(targetUrl).origin;
    const internalLinks = new Set<string>();
  ...
  const pageContents: DeepScrapePage[] = [];
  ...
  // AI Analysis (optional, used by API route; safe for chat too)
...
2. Audience (B2B/B2C, industry, size)
3. Main problems solved (Top 3)
4. Unique selling points (Top 3)
5. Common objections/questions
6. Content gaps on the website
7. Best bot use case
8. Hidden opportunities (from documents)

Website + Documents:\n${fullContext}

Answer in strict JSON with keys: description, audience, problems, usp, objections, contentGaps, recommendedBotType, hiddenOpportunities`;
